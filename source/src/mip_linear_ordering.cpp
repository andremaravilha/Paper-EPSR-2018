...
std::tuple<orcs::Schedule, double> orcs::MIPLinearOrdering::solve(const Problem& problem,
    ...
    const auto& action = problem.action;
    ...
    try {
        ...
        if (warm_start) {
            ...
            for (int idx = 0; idx < schedule[0].size(); ++idx) {
            ...
            for (int l = 1; l <= m; ++l) {
                if (schedule[l].size() > 0) {
                    for (int idx = 0; idx < schedule[l].size(); ++idx) {
                        int i = std::get<ATTR_SWITCH>(schedule[l][idx]);
                        y[i][l].set(GRB_DoubleAttr_Start, 1.0);
                        t[i].set(GRB_DoubleAttr_Start, std::get<ATTR_MOMENT>(schedule[l][idx]));
                        for (int idx_next = idx + 1; idx_next < schedule[l].size(); ++idx_next) {
                            int j = std::get<ATTR_SWITCH>(schedule[l][idx_next]);
        ...
        for (int i = 1; i <= n; ++i) {
            if (technology[i] != Technology::REMOTE) {
                ...
                model.addConstr(t[i] >= t[0] + p[0] + expr);
            }
        }
